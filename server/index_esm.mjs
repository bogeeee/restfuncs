...
// Despite the dual-package hazard, we could not go the esm-only way, ditching commonjs (might be ok, because in the examples for new projects, only ESM is advertised), because: (could not reproduce it!) "tspc (opposed to tsc) needs to also see a commonjs version of it. Strange."
// Publishing as dual (duplicate code) package also does not work at runtime with node then, after successful compile with tspc: SyntaxError: The requested module 'restfuncs-server' does not provide an export named 'ServerSession'. See the exmple project. UPDATE: May be this was caused by: generated files did not have .mjs extension (+ import local files with .mjs extension)
